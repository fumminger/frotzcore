...
public static class OS
{
    ...
    private static IZScreen? XScreen;

    public static void SetScreen(IZScreen screen)
    {
        if (OS.XScreen != null)
        {
            OS.XScreen.KeyPressed -= new EventHandler<ZKeyPressEventArgs>(Screen_KeyPressed);
        }
    ...
    private static void OnFatalError(string message) => XScreen?.HandleFatalError(message);
    ...
    public static void Fail(string message) => XScreen?.HandleFatalError(message);
    ...
    public static void EraseArea(int top, int left, int bottom, int right, int win)
    {
        ...
        {
            XScreen?.ClearArea(top, left, bottom, right);
        }
        else
        {
            XScreen?.ClearArea(top, left, bottom, right);
...
#pragma warning restore CS8763 // A method marked [DoesNotReturn] should not return.
    ...
    public static bool FontData(int font, ref zword height, ref zword width) =>
        XScreen?.GetFontData(font, ref height, ref width) ?? false;
    ...
    public static bool ReadFileName([NotNullWhen(true)] out string? file_name, string default_name, FileTypes flag)
    {
        switch (flag)
        {
            case FileTypes.FILE_SAVE:
                file_name = XScreen?.OpenNewOrExistingFile(FastMem.SaveName, "Choose save game file", "Save Files (*.sav)|*.sav", ".sav");
                break;
            case FileTypes.FILE_RESTORE:
                file_name = XScreen?.OpenExistingFile(FastMem.SaveName, "Choose save game to restore", "Save Files (*.sav)|*.sav");
                break;
            case FileTypes.FILE_SCRIPT:
                file_name = XScreen?.OpenNewOrExistingFile(General.DEFAULT_SCRIPT_NAME, "Choose Script File", "Script File (*.scr)|*.scr", ".scr");
                break;
            case FileTypes.FILE_RECORD:
                file_name = XScreen?.OpenNewOrExistingFile(default_name, "Choose File to Record To", "Record File(*.rec)|*.rec", ".rec");
                break;
            case FileTypes.FILE_PLAYBACK:
                file_name = XScreen?.OpenExistingFile(default_name, "Choose File to playback from", "Record File(*.rec)|*.rec");
                break;
    ...
    public static void InitScreen()
    {
        ...
        _metrics = XScreen?.GetScreenMetrics() ?? default;
    ...
    public static bool ProcessArguments(ReadOnlySpan<string> args)
    {
        ...
        if (args.Length == 0)
        {
            var file = XScreen?.SelectGameFile();
            if (!file.HasValue)
                return false;
    ...
    public static zword ReadLine(int max, Span<zword> buf, int timeout, int width, bool continued)
    {
        ...
        XScreen.GetColor(out int foreground, out int background);
        XScreen.SetInputColor();

        XScreen.SetInputMode(true, true);

        var p = XScreen.GetCursorPosition();
            ...
            while (true)
            {
                ...
                else if (c == CharCodes.ZC_BACKSPACE)
                {
                    if (buffer.Count > 0)
                    {
                        ...
                        XScreen.SetCursorPosition(p.X, p.Y);

                        XScreen.RemoveChars(1);
                        XScreen.RefreshScreen();
                    }
                }
                else if (c == '\t')
                ...
                else
                {
                    ...
                    int w = XScreen.GetStringWidth(((char)c).ToString(), new CharDisplayInfo(ZFont.TEXT_FONT, ZStyles.NORMAL_STYLE, -1, -1));
                    p = p with { X = p.X + w };
                    XScreen.SetCursorPosition(p.X, p.Y);

                    buffer.Add(new(c, w));

                    XScreen.AddInputChar((char)c);
        ...
        {
            XScreen.SetColor(foreground, background);
            XScreen.SetInputMode(false, false);
    ...
    public static zword ReadKey(int timeout, bool cursor)
    {
        ...
        var (x, y) = XScreen.GetCursorPosition();
        ...
        try
        {
            ...
            while (true)
            {
                ...
                lock (Entries)
                {
                    ...
                    int width = XScreen.GetStringWidth(((char)c).ToString(),
    ...
    public static void ScrollArea(int top, int left, int bottom, int right, int units)
    {
        ...
        {
            XScreen.ScrollArea(top, bottom, left, right, units);
        }
        else
        {
            Debug.Assert(units > 0);
            XScreen.ScrollLines(top, bottom - top + 1, units);
        }
    }
     ...
     *
     */
    public static void SetColor(int new_foreground, int new_background) => XScreen?.SetColor(new_foreground, new_background);
    ...
    public static void SetCursor(int row, int col)
    {
        ...
        XScreen?.SetCursorPosition(col, row);
    }
     ...
     *
     */
    public static void SetFont(int newFont) => XScreen?.SetFont(newFont);
     ...
     *
     */
    public static void SetTextStyle(int newStyle) => XScreen?.SetTextStyle(newStyle);
    ...
    public static int StringWidth(ReadOnlySpan<zword> s)
    {
        ...
        return XScreen.GetStringWidth(sb.ToString(), new CharDisplayInfo(font, style, 0, 0));
    }
    ...
    public static void DrawPicture(int picture, int y, int x)
    {
        ...
        {
            XScreen?.DrawPicture(picture, BlorbFile.Pictures[picture].Image, y, x);
        }
    }
     ...
     *
     */
    public static void FinishWithSample(int number) => XScreen?.FinishWithSample(number);
     ...
     *
     */
    public static void PrepareSample(int number) => XScreen?.PrepareSample(number);
    ...
    public static void StartSample(int number, int volume, int repeats, zword eos)
    {
        ...
        XScreen?.StartSample(number, volume, repeats, eos);
     ...
     *
     */
    public static void StopSample(int number) => XScreen?.StopSample(number);
    ...
    {
        bool shouldWrap = XScreen?.ShouldWrap() ?? false;
    ...
    public static void SetWindowSize(int win, ZWindow wp)
        => XScreen?.SetWindowSize(win, wp.YPos, wp.XPos, wp.YSize, wp.XSize);
